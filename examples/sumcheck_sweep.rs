...
struct Args {
  ...
  /// Methods to benchmark (comma-separated). Default: all
  #[arg(
    long,
    value_enum,
    default_value = "base,i32,i64",
    value_delimiter = ','
  )]
...
struct BenchMethods {
  base: bool,
  split_eq_delayed_modular_reduction: bool,
  i32_small: bool,
  i64_small: bool,
}

impl BenchMethods {
  fn from_args(methods: &[SumcheckMethod]) -> Self {
    Self {
      base: methods.contains(&SumcheckMethod::Base),
      split_eq_delayed_modular_reduction: methods.contains(&SumcheckMethod::SplitEqDmr),
      i32_small: methods.contains(&SumcheckMethod::I32),
      i64_small: methods.contains(&SumcheckMethod::I64),
    }
  }
  ...
  fn without_i32(&self) -> Self {
    Self {
      base: self.base,
      split_eq_delayed_modular_reduction: self.split_eq_delayed_modular_reduction,
      i32_small: false,
...
struct TrialResult {
  ...
  split_eq_delayed_modular_reduction: Option<BenchResult>,
...
/// Returns (base_result, split_eq_delayed_modular_reduction_result, i64_result) as Options based on selected methods.
  ...
  E::Scalar: SmallValueField<i64, IntermediateSmallValue = i128> + DelayedReduction<i64>,
{
  ...
  let (result_split_eq_delayed_modular_reduction, r2, evals2) =
  ...
  // Verify split_eq_delayed_modular_reduction matches base (only when both methods were run)
  if let (Some(r1), Some(r2)) = (&r1, &r2) {
    assert_eq!(
      r1, r2,
      "split_eq_delayed_modular_reduction challenges must match base"
    );
  }
  if let (Some(e1), Some(e2)) = (&evals1, &evals2) {
    assert_eq!(
      e1, e2,
      "split_eq_delayed_modular_reduction final evaluations must match base"
    );
  }
  ...
  (
    result_base,
    result_split_eq_delayed_modular_reduction,
    result_i64,
  )
}
...
fn build_csv_header(methods: &BenchMethods) -> String {
  ...
  if methods.split_eq_delayed_modular_reduction {
    ...
    cols.push("split_eq_delayed_modular_reduction_prove_us");
  }
  ...
  if methods.base && methods.split_eq_delayed_modular_reduction {
    cols.push("prove_speedup_split_eq_delayed_modular_reduction");
  }
...
fn format_csv_row(result: &TrialResult, methods: &BenchMethods) -> String {
  ...
  if methods.split_eq_delayed_modular_reduction {
    let r = result.split_eq_delayed_modular_reduction.unwrap();
    vals.push(r.setup_us.to_string());
    vals.push(r.prove_us.to_string());
  }
  if methods.i32_small {
  ...
  if methods.base && methods.split_eq_delayed_modular_reduction {
    ...
    let delayed_modular_reduction_prove =
      result.split_eq_delayed_modular_reduction.unwrap().prove_us as f64;
    let speedup = if delayed_modular_reduction_prove > 0.0 {
      base_prove / delayed_modular_reduction_prove
    } else {
      f64::INFINITY
    };
    ...
    + DelayedReduction<i64>,
{
  ...
  for num_vars in min_vars..=max_vars {
    ...
    for trial in 1..=num_trials {
      let (base_r, split_eq_delayed_modular_reduction_r, i64_r) =
        run_single_benchmark::<E>(num_vars, methods);
      ...
      let result = TrialResult {
        ...
        trial,
        base: base_r,
        split_eq_delayed_modular_reduction: split_eq_delayed_modular_reduction_r,
        i32_small: i32_r,
        i64_small: i64_r,
      };

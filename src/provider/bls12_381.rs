...
mod tests {
  ...
  fn test_scalar_multiplication() {
    ...
    // 0*g = identity
    assert!(
      bool::from((g * g1::Scalar::ZERO).is_identity()),
      "scalar mul by 0"
    );
  ...
  fn test_msm_correctness_against_naive() {
    ...
    let scalars: Vec<g1::Scalar> = (0..n).map(|i| g1::Scalar::from((i + 1) as u64)).collect();
  ...
  fn test_transcript_repr_different_points_differ() {
    ...
    let bytes1: Vec<u8> = <g1::Point as TranscriptReprTrait<g1::Point>>::to_transcript_bytes(&g);
    let bytes2: Vec<u8> =
      <g1::Point as TranscriptReprTrait<g1::Point>>::to_transcript_bytes(&two_g);
  ...
  fn test_to_coordinates() {
    ...
    assert_eq!(
      y_sq, x_cubed_plus_b,
      "generator must satisfy curve equation"
    );
  }

  ...
  fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
    ...
    impl<'de, E: Engine> Visitor<'de> for ArgVisitor<E> {
      ...
      fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        ...
        let proof_bytes: Vec<u8> = seq
          .next_element()?
          .ok_or_else(|| A::Error::custom("missing proof"))?;
        let value_bytes: Vec<u8> = seq
          .next_element()?
...
impl<E: Engine> PCSEngineTrait<E> for DoryPCS<E> {
  ...
  fn setup(
    ...
    params
      .h_gt
      .serialize_compressed(&mut h_gt_bytes)
      .expect("Failed to serialize h_gt");

    let ck = DoryCommitmentKey {
      num_vars,
      h_gt_bytes: h_gt_bytes.clone(),
    };
    let vk = ck.clone();
...
mod tests {
  ...
  fn make_point(n: usize) -> Vec<<E as Engine>::Scalar> {
    (0..n)
      .map(|i| <E as Engine>::Scalar::from(i as u64 * 3 + 7))
      .collect()
  }
  ...
  fn test_dory_setup() {
    ...
    assert_eq!(
      ck.num_vars, vk.num_vars,
      "ck and vk should have same num_vars"
    );
  ...
  fn test_dory_commit() {
    ...
    assert!(
      !comm.unwrap().commitment_bytes.is_empty(),
  ...
  fn test_dory_commitment_deterministic() {
    ...
    assert_eq!(
      comm1.commitment_bytes, comm2.commitment_bytes,
      "Same polynomial should produce same commitment"
    );
  }
  ...
  fn test_dory_prove_verify_complete() {
    ...
    let result = DoryPCS::<E>::verify(&vk, &ck, &mut transcript_v, &comm, &point, &comm_eval, &arg);
  ...
  fn test_dory_reject_wrong_commitment() {
    ...
    assert!(
      result.is_err(),
      "Proof for one commitment must not verify against different commitment"
    );
  }

  #[test]
  fn test_dory_check_commitment_rejects_empty() {
    let empty_comm = DoryCommitment {
  ...
  fn test_dory_rerandomize_preserves_correctness() {
    ...
    assert!(
      result.is_ok(),
      "Proof with original commitment should verify"
    );
    ...
    for i in 0..commitments.len() {
      for j in (i + 1)..commitments.len() {
        assert_ne!(
          ...
          "Commitment {} and {} must be unlinkable",
          i, j
        );
      }
    }

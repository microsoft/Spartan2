...
impl<E: Engine> ReadWriteCheckingSNARK<E> {
  ...
  pub fn prove(
    ...
    let deltas: Vec<Vec<E::Scalar>> = write_addresses[..T - chunk_size]
      .par_chunks_exact(chunk_size)
      .zip(write_increments[..T - chunk_size].par_chunks_exact(chunk_size))
      .map(|(address_chunk, increment_chunk)| {
        let mut delta = vec![E::Scalar::ZERO; K];
        for (&k, &increment) in address_chunk.iter().zip(increment_chunk.iter()) {
          delta[k] += E::Scalar::from_i64(increment);
        }
        delta
      })
      .collect();
    let mut memory_checkpoints = vec![vec![E::Scalar::ZERO; K]];
    for (chunk_index, delta) in deltas.into_iter().enumerate() {
      let next_checkpoint = memory_checkpoints[chunk_index]
        .par_iter()
        .zip(delta.into_par_iter())
        .map(|(&val_k, delta_k)| val_k + delta_k)
        .collect();
      memory_checkpoints.push(next_checkpoint);
    }
    for round in 0..K.log_2() {
      let uni_poly_evals = memory_checkpoints
        .par_iter()
        .zip(write_addresses.par_chunks(chunk_size))
        .zip(write_increments.par_chunks(chunk_size))
        .zip(read_addresses.par_chunks(chunk_size))
        .enumerate()
        .map(|(chunk_idx, (((row, wa_chunk), inc_chunk), ra_chunk))| {
          let mut row = row.to_vec();
          let mut uni_poly_evals = [E::Scalar::ZERO; 3];
          for (j_prime, ((&wa, &ra), inc)) in wa_chunk
            .iter()
            .zip_eq(ra_chunk.iter())
            .zip_eq(inc_chunk)
            .enumerate()
          {
            let j = chunk_idx * chunk_size + j_prime;
            let m = round + 1;

            // Get evals for read-checking sum-check
            let r_k_prime = ra >> m;
            let c_0 = row[2 * r_k_prime];
            let m_c = row[2 * r_k_prime + 1] - row[2 * r_k_prime];
            let c_2 = row[2 * r_k_prime + 1] + m_c;
            let c_3 = c_2 + m_c;
            let k_star = ra % (1 << m);
            let km = k_star >> round;
            ...
            let w_k_prime = wa >> m;
            let w_c_0 = row[2 * w_k_prime];
            let w_m_c = row[2 * w_k_prime + 1] - row[2 * w_k_prime];
            let w_c_2 = row[2 * w_k_prime + 1] + w_m_c;
            let w_c_3 = w_c_2 + w_m_c;
            let eq_r_address_evals = A.sumcheck_evals(w_k_prime, DEGREE, BindingOrder::LowToHigh);
            let w_k_star = wa % (1 << m);
            let w_k_d = w_k_star % (1 << round);
            let w_km = w_k_star >> round;
            let wa_0 = D[w_k_d] * eq_km_c[w_km][0];
            let wa_2 = D[w_k_d] * eq_km_c[w_km][1];
            let wa_3 = D[w_k_d] * eq_km_c[w_km][2];

            // Add read-checking and write checking contributions
            uni_poly_evals[0] += read_comb_func(B[j], ra_evals[0], c_0)
              + z * write_comb_func(eq_r_address_evals[0], B[j], wa_0, wv[j], w_c_0);
            uni_poly_evals[1] += read_comb_func(B[j], ra_evals[1], c_2)
              + z * write_comb_func(eq_r_address_evals[1], B[j], wa_2, wv[j], w_c_2);
            uni_poly_evals[2] += read_comb_func(B[j], ra_evals[2], c_3)
              + z * write_comb_func(eq_r_address_evals[2], B[j], wa_3, wv[j], w_c_3);

            // Update val
            let w_prime = wa >> round;
            let w_double_prime = wa % (1 << round);
            row[w_prime] += E::Scalar::from_i64(*inc) * D[w_double_prime];
          }
          uni_poly_evals
        })
        .reduce(
      ...
      let deltas: Vec<Vec<E::Scalar>> = write_addresses[..T - chunk_size]
        ...
        .map(|(address_chunk, increment_chunk)| {
          let mut delta = vec![E::Scalar::ZERO; K];
          for (wa, &increment) in address_chunk.iter().zip(increment_chunk.iter()) {
            let w_prime = wa >> m;
            let w_double_prime = wa % (1 << m);
            delta[w_prime] += E::Scalar::from_i64(increment) * D[w_double_prime];
          }
          delta
        })
        .collect();
      memory_checkpoints = vec![vec![E::Scalar::ZERO; K >> (round + 1)]];
      for (chunk_index, delta) in deltas.into_iter().enumerate() {
        let next_checkpoint = memory_checkpoints[chunk_index]
          .par_iter()
          .zip(delta.into_par_iter())
    ...
    let val_r_address: Vec<E::Scalar> = memory_checkpoints
      .par_iter()
      .zip(write_addresses.par_chunks(chunk_size))
      .zip(write_increments.par_chunks(chunk_size))
      .flat_map(|((checkpoint, wa_chunk), inc_chunk)| {
        let mut val: Vec<E::Scalar> = Vec::with_capacity(chunk_size);
        val.push(checkpoint[0]);
        for j in 0..(chunk_size - 1) {
          let (k, inc) = (wa_chunk[j], inc_chunk[j]);
          val.push(val[j] + E::Scalar::from_i64(inc) * D[k]);
        }
        val
      })
      .collect();
  ...
  pub fn verify(
    ...
    let (sumcheck_claim, mut r_sumcheck) = self.sumcheck_proof.verify(
    ...
    let (r_address, r_cycle) = r_sumcheck.split_at_mut(K.log_2());
    r_cycle.reverse();
    r_address.reverse();
